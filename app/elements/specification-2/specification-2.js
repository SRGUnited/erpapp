...
Polymer({
  ...
  FnSearchEnquiry:function(e){
      ...
      if(len>1)
        {
              ...
              {
              var subval=((item[i].specification_2_name).trim()).substring(0,backsubval.length);
              if((item[i].specification_2_name).toUpperCase().indexOf((this.value).toUpperCase())!=-1)
              {
              var obj={"itemdes":""};;
              obj.itemdes=item[i].specification_2_name;
              obj.specification_2_id=item[i].specification_2_id;
                  // var obj1={"itemdes":obj.specification_2_name};
              arr.push(obj);
              }
              }
              this.itemArray=arr;
      ...
      //while typing item display
        ...
        for(var i=0;i<item.length;i++)
        {
        var subval=((item[i].specification_2_name).trim()).substring(0,this.itemval.length);
        if(this.itemval == subval)
        {
        if((item[i].specification_2_name).toUpperCase().indexOf((this.itemval).toUpperCase())!=-1)
        {
        var obj={"itemdes":""};
        obj.itemdes=item[i].specification_2_name;
        obj.specification_2_id=item[i].specification_2_id;
  ...
  spec2autocompletefunction:function(e)
        {
           item=e.detail.response.returnval;
           alert(JSON.stringify(item));
        },
  spec2labelinfo:function(e)
      {
        var jsonlabel=this.jsondata;
        this.graderange=jsonlabel[0].grade;
    }
  });
})();
