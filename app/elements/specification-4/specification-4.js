...
Polymer({
  ...
  FnSearchEnquiry:function(e){
      ...
      if(e.keyCode==8){
        ...
        if(len>1){
          ...
          {
            var subval=((item[i].specification_4_name).trim()).substring(0,backsubval.length);
            if((item[i].specification_4_name).toUpperCase().indexOf((this.value).toUpperCase())!=-1)
            {
              var obj={"itemdes":""};;

              obj.itemdes=item[i].specification_4_name;
              obj.specification_4_id=item[i].specification_4_id;
              // var obj1={"itemdes":obj.specification_4_name};
      ...
      if(e.keyCode!=8&& e.keyCode!=16&& e.keyCode!=13 && e.keyCode!=38&&e.keyCode!=40&&e.keyCode!=37&&e.keyCode!=39)
      {
        ...
        if(this.itemval.length>0)
        {
          for(var i=0;i<item.length;i++)
          {
            var subval=((item[i].specification_4_name).trim()).substring(0,this.itemval.length);
           if(this.itemval == subval)
           {
            if((item[i].specification_4_name).toUpperCase().indexOf((this.itemval).toUpperCase())!=-1)
            {
              var obj={"itemdes":""};
              obj.itemdes=item[i].specification_4_name;
              obj.specification_4_id=item[i].specification_4_id;
  ...
  spec4autocompletefunction:function(e){
     item=e.detail.response.returnval;
    alert(JSON.stringify(item));

  },
  spec4labelinfo:function(e){
    var jsonlabel=this.jsondata;
    // alert(jsonlabel[0].specification_4_name);
    this.otherrange=jsonlabel[0].other;
  }
